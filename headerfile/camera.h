...
class camera {
  ...
  private:
    ...
    color ray_color(const ray& r, int depth, const hittable& world) const {
        // If we've exceeded the ray bounce limit, no more light is gathered. prevent stack overflow
        if (depth <= 0)
            return color(0,0,0);
        hit_record rec;
        //we choose the precision of the origin of the bounce it should be above the surface
